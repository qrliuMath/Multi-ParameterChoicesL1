function [u1,TargetValue] = Wavelet_FPPA(c,paraFPPA)
...
for k = 1:MaxIter
    ...
    % v update
    temp = 1/rho*v0 + waverec(2*u1-u0,L,WaveName);
    ...
    TargetValue(k) = 1/2*sum((waverec(u0,L,WaveName)-c).^2) + group_norm;  % Targeted Function value
    if rem(k,100)==0
