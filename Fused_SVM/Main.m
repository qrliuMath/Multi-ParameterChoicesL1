...
paraFPPA.TargetTSLs = [150,120];     %  targeted sparsity levels
paraFPPA.TargetSSL = sum(paraFPPA.TargetTSLs);
paraFPPA.MaxIter = 200000;
...
for LastTag = 0:1:NumExp
    fprintf('\n-------------------- Tag=%d -------------------\n',LastTag)
    [w,a,c,TargetValue] = HingeLoss_FPPA(N,NumTrain,B_tilde,alpha_B_tilde_T,E,paraFPPA);
    ...
    for j = 1:paraFPPA.num_group
        ...
        elseif Result.SLs(j) > paraFPPA.TargetTSLs(j)
            ...
            for i = 1:100
                ...
                [w,a,c,TargetValue] = HingeLoss_FPPA(N,NumTrain,B_tilde,alpha_B_tilde_T,E,paraFPPA);
                Result.SLs(j) = nnz(w(paraFPPA.group_info(j):paraFPPA.group_info(j+1)-1));
                Result.NUM = Result.NUM + 1;
                fprintf('lambda_star = %f\n', paraFPPA.lambda)
                fprintf('SLs = [%d, %d]\n', Result.SLs)
